...
def generate_icons():
    source_path = "assets/icon.png" if os.path.exists("assets/icon.png") else "public/logo.png"
    ...
    for folder, size in sizes.items():
        ...
        # Create a SOLID WHITE canvas of the target size
    ...
    # Generate Adaptive Icon Foreground (432x432, logo scaled to 260x260 in center on TRANSPARENT canvas)
    anydpi_folder = os.path.join(res_dir, "mipmap-anydpi-v26")
    if not os.path.exists(anydpi_folder):
        os.makedirs(anydpi_folder)
    
    # Create 432x432 TRANSPARENT canvas so Android's background layer shows behind it
    adaptive_bg = Image.new("RGBA", (432, 432), (0, 0, 0, 0))
    
    # Scale logo to 260x260 (safe zone for adaptive icons)
    logo_scaled = img.resize((260, 260), Image.Resampling.LANCZOS)
    
    # Paste centered with mask
    offset = ((432 - 260) // 2, (432 - 260) // 2)
